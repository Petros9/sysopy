...
void create_block(char *file_name, array_of_blocks *main_array)
{
    ...
    int index = 0;
    while (index < main_array->blocks_number && main_array->blocks[index] != NULL)
    {
        index++;
    }
    
    //tu jest błąd z tym przypisaniem
    //może nie błąd, ale tu -O2 ma problem
